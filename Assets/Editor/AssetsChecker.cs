...
public class ResourceChecker : EditorWindow {
    ...
    //
    string inputPath = "Assets/Resources";
    //
	...
	void OnGUI ()
	{
		...
		if (GUILayout.Button("Calculate",GUILayout.Width(80), GUILayout.Height(20)))
			CheckResources();
        ...
        //////
        GUILayout.Label("Current Path: " + inputPath);
        //////
	...
	private T[] FindObjects<T>() where T : Object
	{
        ...
        {
            //Put all assets under the resource folder to use LoadAll function
            objects = Resources.LoadAll(inputPath, typeof(T));

            /*
            var dirctory = new DirectoryInfo(inputPath);
            var files = dirctory.GetFiles("*", SearchOption.AllDirectories);
            for (int i = 0; i < files.Length; i++)
            {
               
                if (files[i].Name.EndsWith(".meta")) continue;
                if (files[i].Name.EndsWith(".unity"))
                {
